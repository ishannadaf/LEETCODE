"""

19. Remove Nth Node From End of List

🧾 Problem Statement

Given a linked list, remove the n-th node from the end and return the head.

Input:
1 → 2 → 3 → 4 → 5,  n = 2

Output:
1 → 2 → 3 → 5


⚡ Optimal Approach (One Pass – Two Pointer)
💡 Idea:
Move fast pointer n steps ahead
Then move both slow and fast
When fast reaches end → slow is before target

"""

class ListNode:
    def __init__(self, val=0, next=None):
        self.val = val
        self.next = next


def build_list(arr):
    if not arr:
        return None
    head = ListNode(arr[0])
    curr = head
    for x in arr[1:]:
        curr.next = ListNode(x)
        curr = curr.next
    return head


def print_list(head):
    while head:
        print(head.val, end=" -> ")
        head = head.next
    print("None")





class SolutionOptimal:
    def removeNthFromEnd(self, head, n):
        dummy = ListNode(0)
        dummy.next = head
        
        slow = dummy
        fast = dummy
        
        # move fast n steps ahead
        for _ in range(n):
            fast = fast.next
        
        # move both
        while fast.next:
            slow = slow.next
            fast = fast.next
        
        # delete node
        slow.next = slow.next.next
        
        return dummy.next
    
    
# 🧪 Test
arr = [1,2,3,4,5]
n = 2

head1 = build_list(arr)
head2 = build_list(arr)

# res1 = SolutionBrute().removeNthFromEnd(head1, n)
res2 = SolutionOptimal().removeNthFromEnd(head2, n)

# print("Brute Force:")
# print_list(res1)

print("Optimal:")
print_list(res2)

"""

Time: O(n) (1 passes)
Space: O(1)


🎯 Key Concepts
Dummy node avoids edge cases (like deleting head)
Fast pointer creates a gap of n
Slow pointer reaches node before deletion
🧠 One-Line Understanding

👉 Maintain a gap of n nodes → when fast ends, slow deletes target

🚨 Common Mistakes
Not using dummy → fails when deleting head
Moving fast incorrectly
Off-by-one errors
"""